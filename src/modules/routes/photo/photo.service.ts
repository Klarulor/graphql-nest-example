...
export class PhotoService{
  ...
  public async create(@Body() dto: CreatePhotoRequestDTO, creator: User|undefined|null): Promise<CreatePhotoResponseDTO>{
    const photo = new Photo();
    photo.imageUrl = dto.imageUrl;
    photo.creator = creator ?? undefined;
    await photo.save();
    return {id: photo.id};
  }

  public async update(id: number, dto: UpdatePhotoRequestDTO, user: User|undefined|null): Promise<void>{
    const target = await Photo.findOneBy({id});
    if(!target) throw `No photo found with ${id} id`;
    if(user && target.creator?.id != user.id) throw `You have no access to this rating`;
    target.imageUrl = dto.imageUrl;
    await target.save();
  }

  public async delete(id: number, user: User|null): Promise<void|undefined|null>{
    const target = await Photo.findOneBy({id});
    if(!target) throw `No photo found with ${id} id`;
    if(user && target.creator?.id != user.id) throw `You have no access to this rating`;
    await target.remove();
